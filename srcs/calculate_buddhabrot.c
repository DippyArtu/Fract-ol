...
//TODO sick now try to move to opencl, if doesnt work, use multithreading instead
//TODO watch out for cl_complex.h!

//TODO in CL Kernel I seem to be able to send over the memory area via struct, but data contained isn't delivered
//TODO I can send over area for obrits and samples, but not samples themselves
//TODO Maybe try to create an OpenCL RNG so that I dont have to pregen and send samples?
//TODO also look up Mmap for reading from file
...
static t_complex 			**alloc_orbits(int global_size, int size)
{
	t_complex 				**orbits;
	int 					i;
	int 					j;
	...
	i = 0;
	if (!(orbits = (t_complex **)malloc(global_size * sizeof(t_complex *))))
		error(3);
	while (i < global_size)
	{
		if (!(orbits[i] = (t_complex *)malloc(size * sizeof(t_complex))))
			error(3);
		i++;
	}
	i = 0;
...
static void 				free_orbits(t_complex **orbits, int size)
{
	int 					i;
...
//TODO heatmap should be size of ndrange, executed for n_smaples range
//TODO create buffs for samples
void 						buddhabrot(t_fract *fract, t_cl *cl)
{
	...
	map = alloc_heatmap();
	join_threads(fract);
	if (!test_samples(fract))
	{
		ft_putstr("Please try again\n");
		exit(0);
	}
	ft_putstr("Samples generated successfully\n");
	fract->buddha->orbits = alloc_orbits(fract->buddha->n_samples, fract->buddha->max_iter);

	create_buffs(cl, elems, BUDDHA);
	exec_kernel_buddha(cl, fract->buddha->n_samples);

	res = read_buff_buddha(cl, fract->buddha->n_samples);

	printf("done\n");
	printf("%f\n", res[0]);
	printf("%f+%fi\n", cl_creal(fract->buddha->samples[0]), cl_cimag(fract->buddha->samples[0]));
...
//	}
	...
	free_heatmap(map);
	free_orbits(fract->buddha->orbits, fract->buddha->n_samples);
	//free(color);
}
//ndsamples 810000
//809999
...
//	map = alloc_heatmap();
//
//	gen_heatmap(map, fract->buddha, &max_heatmap_val); //move to openCl
//
//	i = 0;
//	row = 0;
//	while (row < B_HEIGHT)
